...
###   This script attempts to download the signature file SHA256SUMS.asc from weycoinproject.org
...
WORKINGDIR="/tmp/weycoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://weycoinproject.org/bin/"
VERSIONPREFIX="weycoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'weycoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the WeyCoin Core binary release signing key installed?"
   fi
