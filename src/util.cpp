...
/**TODO-- port to weycoin*/
//WeyCoin only features
...
const char * const WEYCOIN_CONF_FILENAME = "weycoin.conf";
const char * const WEYCOIN_PID_FILENAME = "weycoin.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
				...
				/**TODO-- */
				// "weycoin" is a composite category enabling all WeyCoin-related debug output
				if(ptrCategory->count(string("weycoin"))) {
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
		/**TODO-- */
        // Create empty weycoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override weycoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", WEYCOIN_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure WeyCoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("WeyCoin Core") == std::string::npos) {
