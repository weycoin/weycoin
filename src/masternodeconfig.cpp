...
bool CMasternodeConfig::read(std::string& strErr) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            CWeyCoinAddress address(donationAddress);
            if (!address.IsValid()) {
                strErr = "Invalid WeyCoin address in masternode.conf line: " + line;
                streamConfig.close();
                return false;
            }
        }
        ...
        if(port != 11526 && Params().NetworkIDString() == CBaseChainParams::MAIN) {
            ...
            strErr = "Invalid port detected in masternode.conf: " + line + " (must be 10526 for testnet)";
            streamConfig.close();
            return false;
        }
