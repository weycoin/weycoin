...
*Setup instructions for a gitian build of WeyCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the WeyCoin
...
- [Building WeyCoin](#building-weycoin)
...
Clone the git repositories for weycoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/weycoin/weycoin
```
...
In [doc/release-process.md](release-process.md) in the weycoin repository under 'Fetch and build inputs'.
...
To build WeyCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the weycoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/weycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/weycoin/weycoin
...
For example:
```bash
URL=https://github.com/laanwj/weycoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit weycoin=${COMMIT} --url weycoin=${URL} ../weycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit weycoin=${COMMIT} --url weycoin=${URL} ../weycoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/weycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/weycoin-build.assert
```
...
[weycoin/gitian.sigs](https://github.com/weycoin/gitian.sigs/) repository, or
